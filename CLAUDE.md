...
3. テスト状況を確認する
   - npm run test を実行してパス状態を把握する
   - Rust ファイルを触る場合は cargo test も実行する
```

**チェック完了後、以下の形式でユーザーに報告してから作業に入ること（報告なしに作業開始することは禁止）:**

```
### セッション開始チェック完了
- ARCHITECTURE.md: 確認済み（最終更新: YYYY-MM-DD）
- テスト: X passed ✅ / ❌ X failed（失敗がある場合は内容を列挙）
- 気になった点: （あれば記載、なければ「なし」）
```
...
バグ修正・レビュー指摘対応・小さなリファクタリングであっても、
実装に入る前に「何をどう変えるか」を1〜2文でユーザーに伝えて確認を取ること。

- ✅ 「`useProjectSummary` からカウントを取得することで0件表示を解消します。よいですか？」
- ❌ 確認なしにファイルを編集し始める（たとえ1行の修正であっても禁止）

プランモードは不要。ただしサイレントに即実装することは禁止。

---

## 進捗報告（必須）

タスクリストの各ステップ完了時に必ず「✅ ステップ名 完了」と報告する。
サイレントに次工程へ進むことは禁止。
...
1. プランモードで作業範囲を確認・承認する（必須）
   ...
   - **プランファイルは必ず `## タスクリスト` セクションで終わること（下記テンプレート参照）**
...
4. テストを書いてから実装する（TDD）
   ...
   - **タスクリストでは「テスト追加（Red確認）」と「実装（Green確認）」を必ず別ステップに分ける**
   - `/tdd-guard` を使うと TDD 順序を自動検証できる（テストより先に実装コミットがある場合に警告）
...
> **進捗報告**: 各ステップ完了時に「✅ ステップ名 完了」とひとこと報告する。
